...
const Navbar = () => {
  ...
  // Close search input when route changes
  useEffect(() => {
    setSearchOpen(false);
    setIsOpen(false);
  }, [location.pathname]);
  ...
  return (
    <div className="fixed top-0 left-0 z-50 w-full flex items-center justify-between px-6 md:px-16 lg:px-36 py-5 bg-black/50 backdrop-blur-md shadow-md">
      ...
      >
        ...
        <div
          className={`fixed top-0 left-0 h-full w-3/4 bg-gray-900/95 backdrop-blur-md flex flex-col p-8 gap-8 transition-transform ${
            isOpen ? "translate-x-0" : "-translate-x-full"
          }`}
        >
          <XIcon
            className="w-6 h-6 text-white cursor-pointer self-end"
            onClick={() => setIsOpen(false)}
          />
          ...
          >
            ...
            {favoriteMovies.length > 0 && (
              <span className="bg-primary text-black rounded-full px-2 ml-2 text-sm">
